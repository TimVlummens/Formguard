// Taken from https://github.com/asumansenol/double_edged_sword_crawler/blob/main/helpers/register_login_feature_extraction/register_login_regexes.js
...
// Some constants were converted to global variables (`window.X = ...`) to be used in other puppeteer scripts via `Page.evaluate`.

/**
 * @param {string[]} vocabs
 * @returns {string}
 */
concatRegexStr = function(vocabs) {
  let regexSource = '(';
 ...
 * @returns {RegExp}
 */
createRegexpFromSrc = function(source) {
 ...
 * @returns {RegExp}
 */
createExactMatchRegexpFromSrc = function(source) {
 ...
 * @returns {RegExp}
 */
createRegexInclucingTwoSrc = function(source1, source2) {
 ...
 */
window.loginActionRegexPattern = createRegexpFromSrc(concatRegexStr(loginActionVocabs));

/**
 * Multiple translations of the RegExp login pattern.
 */
window.loginRegexPattern = createRegexpFromSrc(concatRegexStr(loginVocabs));
 ...
 */
window.registeRegexPattern = createRegexpFromSrc(concatRegexStr(registerVocabs));

/**
 * RegExp pattern for register actions.
 */
window.registerActionRegex =
