...
(function (global, factory) {
    ...
    function *detectEmailInputs(domRoot) {
        ...
        const typeEmailInputs = Array.from(queryAllDeep(domRoot, "input[type='email']"));
        for (const input of typeEmailInputs) {
            // yield input;
            // send -1 as the score for fields detected based on type
            yield {xpath: getXPath(input), id: input.id, score: -1};
        }
        ...
        for (const input of detectedInputs) {
            console.log({xpath: getXPath(input.element), id: input.element.id, score: input.scoreFor("email")})
            if (input.scoreFor("email") > 0.5) {
                // yield input.element;
                yield {xpath: getXPath(input.element), id: input.element.id, score: input.scoreFor("email")};
            }
        }
    }
