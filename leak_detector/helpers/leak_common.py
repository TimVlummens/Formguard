"""
Taken from https://github.com/leaky-forms/leaky-forms/tree/main/leak-detector
"""
...
f = open (MODULE_DIR+'entities.json', "r", encoding='utf8')
...
def matched_domains(request_url_domain, last_page_domain):
    ...
    req = tldextract.extract(request_url_domain)
    last = tldextract.extract(last_page_domain)
    if (req is None) or (last is None):
        return False
    
    req_domain_without_suffix = req.domain
    last_page_domain_without_suffix = last.domain
